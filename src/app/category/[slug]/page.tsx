...
import { Newspaper, Eye, ChevronLeft } from 'lucide-react'
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Get all category IDs (including children and grandchildren) for articles query
  const allCatIds = [category.id, ...category.children.flatMap(c => [c.id, ...c.children.map(sc => sc.id)])]

  const articles = await prisma.article.findMany({
    where: { categoryId: { in: allCatIds }, status: 'published' },
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full">
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">{category.nameAr}</h1>
              <p className="text-sm text-slate-500">
                {category.nameEn} • {articles.length} مقال
              </p>
            </div>
          </div>
          {category.description && (
            <p className="text-slate-600 mt-3 text-sm">{category.description}</p>
          )}
        </div>
        ...
        {category.children.length > 0 && (
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-slate-500 mb-3">الأقسام الفرعية</h2>
            <div className="flex flex-wrap gap-2">
              {category.children.map(c => (
                <Link
                  key={c.id}
                  href={`/category/${category.slug}#${c.slug}`}
                  className="px-3 py-1.5 bg-white border border-slate-200 rounded-full text-sm text-slate-700 hover:border-slate-900 hover:bg-slate-900 hover:text-white transition-colors"
                >
                  {c.icon} {c.nameAr}
                </Link>
              ))}
          ...
          {articles.map(a => (
            <Link key={a.id} href={`/article/${a.slug}`} className="group">
              <article className="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-md transition-shadow h-full">
                ...
                <div className="p-3">
                  <div className="text-xs text-slate-500 mb-1">{a.category?.nameAr}</div>
